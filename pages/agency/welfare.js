...
const agencyType = '아동복지 센터';

const WelfarePage = ({ teacherDataArr, classDataArr }) => {
...
export async function getStaticProps() {
  ...
  try {
    ...
    const teacherRes = await axios.get(
      `${process.env.NEXT_PUBLIC_URL}/teacher/read?classType=아동복지센터`,
      {
        headers: {
    ...
    const classRes = await axios.get(
      `${process.env.NEXT_PUBLIC_URL}/class/read?classType=아동복지센터`,
      {
        headers: {
          'Content-Type': 'application/json',
        },
